...
void OutFile_png_rgb::setSize(int32_t width, int32_t height) {
    ...
    if(transparent)
        samplesPerPixel = 4;
    fileData = (uint8_t*) allocateMemory(width * samplesPerPixel,
...
bool OutFile_tiff::open() {
    ...
    if (mapProjection == "Equidistant Cylindrical" || 
...
void OutFile_tiff_color::setSize(int32_t width, int32_t height) {
    ...
    if (transparent) {
        fileData = (uint8_t*) allocateMemory(width * 4,
                "OutFile_tiff_color::fileData");
    } else {
...
void OutFile_tiff_color::writeLine() {
    if(transparent) {
        for (int i = 0; i < width; i++) {
            ...
            uint8_t scaledVal = (uint8_t) round(
                productStuff[0]->calcOutputVal(productStuff[0]->lineData[i]));
...
bool OutFile_netcdf4::open() {
    ...
    try {
        ...
        double latStep = (metaData->north - metaData->south)
            / (float) getHeight();
        double lonStep;
        if (metaData->east < metaData->west)
        ...
        if (mapProjection == "Equidistant Cylindrical" // SMI
                || mapProjection == "PlateCarree"
                || (proj4String.find("+proj=eqc") != string::npos) ) {
        ...
        for (size_t prodNum = 0; prodNum < productStuff.size(); prodNum++) {
            ...
            NcVar var = createProduct(pInfo, varDataType, dimIds);
            initCompression(var);
        ...
        if (qualityData) {
            ...
            if (!findProductInfo(qualityName.c_str(), metaData->sensorID, qInfo)) {
                cerr << "-E- OutFile_netcdf4::open - cannot find product "
                     << qualityName << " in product.xml" << endl;
                exit(EXIT_FAILURE);
            }
        ...
        if (!findProductInfo("lat", metaData->sensorID, latInfo)) {
            cerr << "-E- OutFile_netcdf4::open - "
                "cannot find \"lat\" in product.xml" << endl;
            exit(EXIT_FAILURE);
        }
        productInfo_t* lonInfo = allocateProductInfo();
        if (!findProductInfo("lon", metaData->sensorID, lonInfo)) {
            cerr << "-E- OutFile_netcdf4::open - "
                "cannot find \"lon\" in product.xml" << endl;
            exit(EXIT_FAILURE);
        }

        if(fullLatLon == LatLon1D) {

            float* latarray = (float*) allocateMemory(getHeight() * sizeof (float),
                                                      "latarray");
            for (int i = 0; i < getHeight(); i++)
                latarray[i] = metaData->north - latStep * i - latStep / 2.0;
            vector<NcDim> latDim; latDim.push_back(dimIds[0]);
            NcVar lat = createProduct(latInfo, ncFloat, latDim);
            lat.putVar(latarray);
            free(latarray);

            float* lonarray = (float*) allocateMemory(getWidth()  * sizeof (float),
                                                      "lonarray");
            for (int i = 0; i < getWidth(); i++)
                lonarray[i] = metaData->west + lonStep * i + lonStep / 2.0;
            vector<NcDim> lonDim; lonDim.push_back(dimIds[1]);
...
void OutFile_netcdf4::writeLine() {
    ...
    for (size_t prodNum = 0; prodNum < productStuff.size(); prodNum++) {
        ...
        prodVars[prodNum].putVar(start, count, fileData);
    }
...
void OutFile_netcdf4::initCompression(NcVar var) {
    ...
    for (size_t i = 0; i < dims.size(); i++) {
        ...
        if (dims[i].getSize() < 1000) {
            newchunk = floor(dims[i].getSize() / 3) + 1;
        } else {
            newchunk = floor(dims[i].getSize() / 100) + 1;
            if (newchunk > 1500) {
                newchunk = floor(dims[i].getSize() / 250) + 1;
                if (newchunk > 40)
                    newchunk = 40;
            }
        }
        chunksize.push_back(newchunk);
    }
...
NcVar OutFile_netcdf4::createProduct(productInfo_t* pInfo,
                                     const NcType& ncType,
    ...
    if (pInfo->validMin != FLT_MIN || pInfo->validMax != FLT_MAX) {
        var.putAtt("valid_min", ncType,
                   (pInfo->validMin - pInfo->addOffset)/pInfo->scaleFactor);
        var.putAtt("valid_max", ncType,
                   (pInfo->validMax - pInfo->addOffset)/pInfo->scaleFactor);
    }
...
NcType OutFile_netcdf4::getDataType(DataStorage dataStorage) {
    switch (dataStorage) {
    case ByteDS  : return NC_BYTE;   // ncByte
    case UByteDS : return NC_UBYTE;  // ncUbyte
    case ShortDS : return NC_SHORT;  // ncShort
    case UShortDS: return NC_USHORT; // ncUshort
    case IntDS   : return NC_INT;    // ncInt
    case UIntDS  : return NC_UINT;   // ncUint
    case FloatDS : return NC_FLOAT;  // ncFloat
    case DoubleDS: return NC_DOUBLE; // ncDouble
    default:
        cerr << "-E- OutFile_netcdf4::getDataType - illegal data storage type = "
             << dataStorage << endl;
        exit(EXIT_FAILURE);
    }
}
