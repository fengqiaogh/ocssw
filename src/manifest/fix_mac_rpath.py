...
# It also copies gcc libs into OCSSW/opt/lib for seadas distribution
    ...
    for fileName in os.listdir('.'):
        if os.path.isfile(fileName):
            if ".dylib" in fileName:
                ...
                for line in lines:
                    if '/opt/local/lib/' in line or '/opt/X11/lib/' in line or os.environ['LIB3_DIR'] in line or ('/' not in line and 'compatibility' in line):
                        #print('  ' + line)
    ...
    for fileName in os.listdir('.'):
        if os.path.isfile(fileName):
            ...
            if "Mach-O 64-bit executable" in line:
                ...
                lines = out.decode("utf-8").split('\n')
                for line in lines:
                    if '/opt/local/lib/' in line or '/opt/X11/lib/' in line or os.environ['LIB3_DIR'] in line:
                        print('  ' + line)
                        libPath = line.split()[0]
                        parts = libPath.split('/')
                        libName = parts[len(parts)-1]
    ...
    for fileName in os.listdir('.'):
        if os.path.isfile(fileName):
            ...
            if "Mach-O 64-bit executable" in line:
                ...
                lines = out.decode("utf-8").split('\n')
                for line in lines:
                    if '/opt/local/lib/' in line or '/opt/X11/lib/' in line or os.environ['LIB3_DIR'] in line or '%s/lib' % (os.environ['OCSSWROOT']) in line:
                        #print('  ' + line)
                        libPath = line.split()[0]
                        parts = libPath.split('/')
                        libName = parts[len(parts)-1]
