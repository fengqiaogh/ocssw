...
int l2bin_init_options(clo_optionList_t* list, const char* prog, const char* version) {
    ...
    strcpy(tmpStr, "equator crossing time delta in minutes\n");
    ...
    clo_addOption(list, "output_wavelengths", CLO_TYPE_STRING, "ALL", "comma separated list of wavelengths for multi-wavelength products");
...
int l2bin_input(int argc, char **argv, instr *input, const char* prog, const char* version) {
    ...
    input->files = ncfiles(localIfile);
