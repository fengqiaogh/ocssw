...
// #define NON_CACHED 1
// get the a l2 name (not expanded from L2 file)
...
int32_t openL2(const char *fname, const char *plist, l2_prod *l2_str)
{
    ...
    // {
    //     printf("Debug info Alex: fine # and all the products in the file  %d = %s -> %s \n \n",fileindex,prodlist[fileindex],plist);
    // }
    ...
    // {
    //     printf("Debug info Alex : product number %d \n \n",l2_str->nprod);
    //     for(int i=0; i < l2_str->nprod; i ++)
    //     {
    //         printf("Product name I passed = %s and index %d \n\n",l2_str->prodname[i], i);
    //     }
    // }
    ...
    if (fileformat == DS_NCDF)
    {
        ...
        l2_str->wavelength_3d = wavelength_3d; // bands_per_pixel and wavelength_3d are essentiall the size of the third dimension
    }
    else
    {
    ...
    ds_id_wave[fileindex] = (idDS){ds_id.fid,
                                   selectDS(ds_id, "wavelength"),
                                   ds_id.fftype};
    if (ds_id_wave[fileindex].sid != -1)
    {
        readDS(ds_id_wave[fileindex], "wavelength", &zero32, NULL,
               &bandsPerPixel, l2_str->wavelength);
        endaccessDS(ds_id_wave[fileindex]);
    }

    if (fileformat == DS_NCDF)
        ds_id.fid = grp_id[fileindex][0];
    ds_id_wave3d[fileindex] = (idDS){ds_id.fid,
                                     selectDS(ds_id, "wavelength_3d"),
                                     ds_id.fftype};
    if (ds_id_wave3d[fileindex].sid != -1)
    {
        readDS(ds_id_wave3d[fileindex], "wavelength_3d", &zero32, NULL,
               &wavelength_3d, l2_str->wavelengths_3d);
    ...
    for (i = 0; i < l2_str->nprod; i++)
    {
        ...
        if (sds_id != -1)
        {
            getTypeDS(ds_id, l2_prd, &(prodtype[fileindex][i])); // we need a function that converts it // it is a product index
            ...
            if (fldDims[2] == bandsPerPixel)
                ...
                l2_str->thirdDim[i] = get_set_flag() ? 1 : wavelength_3d; //  the size of wavelenght output array
            else
                l2_str->thirdDim[i] = 1;
        }
        else
    ...
    // is allocation happening here? Needs to be checked

    // {
    //     printf("Debug info Allocating the data\n\n");
    // }
...
int32_t readdata_cached(idDS ds_id, char *name,
                        int32_t *start, int32_t *stride, int32_t *edges,
                        void *data, unsigned char *scan_in_rowgroup,
                        cache_str *cache, int32_t dtype)
{
    int32_t start2[3];
    int32_t edges2[3];
    int32_t bscan;
 ...
 */
void interpolatePixelCorners(float *lat0, float *lon0,
 ...
 * @param numPoints number of elements in the input arrays
 */
void extrapolatePixelCorners(float *lat0, float *lon0,
                             float *lat1, float *lon1,
                             float *latOut, float *lonOut,
                             int32_t numPoints)
{
 ...
 * @param numPoints number of elements in the input arrays
 */
void calculatePixelDeltas(float *lat0, float *lon0,
                          float *lat1, float *lon1,
                          float *latOut, float *lonOut,
                          int32_t numPoints)
{
               ...
               unsigned char *scan_in_rowgroup)
{
    ...
    for (i = 0; i < l2_str->nprod; i++)
    {
        ...
        if (scan_in_rowgroup == NULL)
        {
            status = readDS(ds_id_prod[ifile][i], l2_prd,
                            start, NULL, edges,
                            (void *)databuf);
        }
        else
        {
            status = readdata_cached(ds_id_prod[ifile][i], l2_prd,
                                     start, NULL, edges,
    ...
    if (pixelAreaInfo.mode != L2PixelOff)
    {
        ...
        if (pixelAreaInfo.lastLine == -1)
        {
            ...
            if (l2_str->detnum &&
                (l2_str->detnum[recnum + 1] != l2_str->detnum[recnum]) &&
                (abs(l2_str->detnum[recnum + 1] - l2_str->detnum[recnum]) != 1))
            {
                printf("ERROR - %s:%d - the detector number of the next line is not from the next detector.\n", __FILE__, __LINE__);
            ...
            if (pixelAreaInfo.mode == L2PixelCorner)
            {
                extrapolatePixelCorners(pixelAreaInfo.lastLat, pixelAreaInfo.lastLon,
                                        l2_str->latitude, l2_str->longitude,
                                        l2_str->lat1, l2_str->lon1,
                                        l2_str->nsamp);
                interpolatePixelCorners(l2_str->latitude, l2_str->longitude,
                                        pixelAreaInfo.lastLat, pixelAreaInfo.lastLon,
                                        l2_str->lat2, l2_str->lon2,
                                        l2_str->nsamp);
            }
            else
            {
                calculatePixelDeltas(l2_str->latitude, l2_str->longitude,
                                     pixelAreaInfo.lastLat, pixelAreaInfo.lastLon,
                                     l2_str->lat1, l2_str->lon1,
                                     l2_str->nsamp);
            }
        ...
        else
        {
            ...
            if (pixelAreaInfo.mode == L2PixelCorner)
            {
                ...
                {
                    interpolatePixelCorners(pixelAreaInfo.lastLat, pixelAreaInfo.lastLon,
                                            l2_str->latitude, l2_str->longitude,
                                            l2_str->lat1, l2_str->lon1,
                                            l2_str->nsamp);
                }
        ...
        //        printf("plt.plot(lon%d, lat%d, \"ro\")\n", i, i);
                   ...
                   unsigned char *scan_in_rowgroup)
{
    ...
    case 1:
        ...
        if (scan_in_rowgroup == NULL)
        {
            readDS(ds_id_ll[ifile][0], "longitude", start, NULL, edges2,
                   (void *)l2_str->lon_cntl);
            readDS(ds_id_ll[ifile][1], "latitude", start, NULL, edges2,
    ...
    case 2:
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            {
                readDS(ds_id_file[ifile], geonav_name[i], start, NULL, edges2,
                       (void *)geonav[i]);
            }
        }

        geonav_(geonav[0], geonav[1], geonav[2], geonav[3], (int32_t *)&nsta[ifile],
                (int32_t *)&ninc[ifile], (int32_t *)&l2_str->nsamp, l2_str->latitude,
                l2_str->longitude, (float *)databuf,
                (float *)databuf, (float *)databuf,
                (float *)databuf);
