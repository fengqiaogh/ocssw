...
#define VERSION "1.10.05_2023-02-28"
...
int main (int argc, char* argv[])
{
  ...
  if ( argc == 1) {
    cout << endl << 
      ...
      "    [-s | --swir_loff_set SWIR_LOFF_config]\n" <<
  ...
  while (1) {
    ...
    c = getopt_long( argc, argv, "k:s:t:o:p:d:n:f", long_options, &option_index);
  ...
  if ((strcmp(argv[optind+1],"0")!=0) && (time_start.empty()) && (outfname.compare("")!=0)) {
  	cout << "Set granule length parameter to 0 or provide start time when specifying output file name." << endl;
  	exit(1);
  }
  ...
  while ((ancind0 == -1) && !endfile) {
      read_oci_scan_packets( &tfileStream, fpacket, 
                             ...
                             npkts0, spn0, ancind0, tlmind0, noseq, endfile, isSPW);
    }
  if ( endfile) {
  ...
  while (!endfile) {
    ...
    if (mper > 0) {
      ...
      ltime = (int32_t) stime;        // the granule starts with specified start time, instead of rounding to 5 minute intervals. LH, 11/22/2022
      mtime = ltime + mper*60;
    ...
    if (outfname.compare("") == 0) {
		...
		if (!isSPW)  l1a_name += "_" + basenme.substr(0,4) + basenme.substr(5,3) + basenme.substr(9,3);
    ...
    if (isc > 0) {
      ...
      // reopen file to write common global metadata
      set_global_attrs(l1a_name, history, doi);
...
int l1aFile::write_oci_global_metadata( time_struct &starttime,
  ...
  string dtypes[] = {"", "Earth Collect", "Dark Collect", "Solar Cal",
                     "SPCA Cal", "Response Curve", "Lunar Cal",
