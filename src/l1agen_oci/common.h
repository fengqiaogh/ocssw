...
#define SWAP_4(x) ( (((x) << 24) & 0xFF000000) | \
         (((x) << 8) & 0x00FF0000) | \
         (((x) >> 8) & 0x0000FF00) | \
         (((x) >> 24) & 0x000000FF) )

int get_band_dims( uint8_t *apacket, uint16_t &ncp, uint16_t &nbb,
                   uint16_t &nrb,
...
int read_oci_scan_packets( std::fstream *tfileStream, uint8_t *apacket, 
                           uint8_t (*pbuffer)[PKTSIZE], 
                           uint32_t &npkts, int32_t &spnum,
                           int32_t &ancind, std::vector<int32_t> &tlmind,
                           uint8_t &seqerr, int32_t &endfile, bool isSPW);

int anc_compare( uint8_t *apacket0, uint8_t *apacket);

int read_packet( std::fstream *tfileStream, uint8_t *packet, 
                 uint32_t &len, uint32_t &apid, int32_t &endfile, bool isSPW);
