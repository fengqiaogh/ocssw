...
//l1c options--------------------------
    ...
    strcat(tmpStr, "        1: L1C grid creation from L1B granules-SOCEA\n");
    strcat(tmpStr, "        2: SBS/SOCEA discrete binning\n");
    strcat(tmpStr, "        3: weighted binning\n");
    strcat(tmpStr, "        4: L2-L1C binning\n");
    strcat(tmpStr, "        5: L1C grid creation from HKT telemetry\n");
    strcat(tmpStr, "        6: L1C grid creation from L1B granules--Fixed Bearing projection\n");
    strcat(tmpStr, "        7: CTH-corrected L1C grid creation from L2 granules--\n");
    strcat(tmpStr, "        8: L1C grid creation from L1B granules-and input L1C grid SOCEA-L1 readers\n");
    strcat(tmpStr, "        9: CTH-corrected L1C grid creation from L1C granules--\n");
...
//    clo_addOption(list, "sensor", CLO_TYPE_INT, "514", tmpStr);
    clo_addOption(list, "sensor", CLO_TYPE_STRING, "OCI", tmpStr);
    ...
    clo_addOption(list, "start_time", CLO_TYPE_STRING, "2022-03-21T00:00:00Z", tmpStr);

    strcpy(tmpStr, "End time as ISO for processing HKT granules ->L1C files");
    strcat(tmpStr, "  units as ISO ");
    clo_addOption(list, "end_time", CLO_TYPE_STRING, "2022-03-21T00:00:00Z", tmpStr);
    ...
    strcat(tmpStr, "    0: time zero seconds of the day 00:00:00, 1: starting coverage time for the HKT file by default, 0 by default");
    ...
    strcat(tmpStr, "        0: CTH=0 no parallax correction");
    strcat(tmpStr, "        1: CTH=k, constant CTH for L1C");
    strcat(tmpStr, "        2: CTH=k, variable CTH for L1B");
    clo_addOption(list, "cloud_correct", CLO_TYPE_BOOL, "0", tmpStr);
...
int32_t L1C_input::l1c_load_input(clo_optionList_t* list,L1C_input *l1ccli) {
    ...
    for (optionId = 0; optionId < numOptions; optionId++) {
      ...
      //L1C new options-----------------------------------------  
            ...
            l1ccli->cloud_correct = clo_getOptionBool(option);
        }
...
int32_t L1C_input::l1c_input_init(L1C_input *l1ccli) {
    ...
    l1ccli->start_time[0] = '\0';;//as ISO
    l1ccli->end_time[0] = '\0';;//as ISO
    l1ccli->history[0] = '\0';
...
//    cout<<"sensor id.."<<format.sensor_id<<"sensor name.."<<sensorId2SensorName(format.sensor_id)<<endl;
    ...
    if (l1cinput->l1c_pflag==8 || l1cinput->l1c_pflag==9){
//        cout<<" IF OPTION IS 9 reading the # of L1C granules with matching time for L1B input...."<<endl;
        strcpy(localIfile_l1c, clo_getString(list, "l1c_grid"));
        l1cinput->files_l1c = ncfiles(localIfile_l1c); //files stored in a vector container in input struc
