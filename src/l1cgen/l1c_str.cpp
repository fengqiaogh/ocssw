...
static float I_FillValue = -999.0;
static float Ipol_FillValue = -999.0;
...
namespace l1c {
     ...
     l1c_str::l1c_str() :  att_ang{-999.0,-999.0,-999.0}, orb_pos{-999.0,-999.0,-999.0}, orb_vel{-999.0,-999.0,-999.0} {  
       //global attributes
   npix=-1;
   iscan=0;
   nscan=-1;
...
//l1cfile assig
    ...
    cout<<"n_views..."<<l1cfile->n_views<<"intensity bands.per view..."<<nband_view<<"polarization bands.per view."<<npol_band_view<<endl;
...
//        status = nc_get_var_double(groupid, varid, scan_time);
    ...
    for(size_t i=0; i<num_scans; i++) {
        if(l1cstr->timepix[i] == scan_timeFillValue)
            l1cstr->timepix[i] = BAD_FLT;     
    }
...
//GEOLOCATION
  ...
  status = nc_get_vara_float(geolocationGrp, senaId, start, count, l1cstr->senazpix);//sensor azimuth -1800 to 1800 degrees, so must be divdied by 100
...
//RADIANCES
  ...
  status = nc_get_vara_float(observationGrp, IntId, start3, count2, l1cstr->I[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);
...
//RADIANCES
  ...
  status = nc_get_vara_float(observationGrp, Lt_blueId, start3, count2, l1cstr->Lt_blue[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);

  count2[0] = num_red_bands;
  status = nc_get_vara_float(observationGrp, Lt_redId, start3, count2, l1cstr->Lt_red[0]);
  check_err(status, __LINE__, __FILE__);

  count2[0] = num_SWIR_bands;
  status = nc_get_vara_float(observationGrp, Lt_SWIRId, start3, count2, l1cstr->Lt_SWIR[0]);
  check_err(status, __LINE__, __FILE__);
...
//RADIANCES
  ...
  status = nc_get_vara_float(blueGrp, senaId, start3, count2, l1cstr->senazpix_3d[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);
  status = nc_get_vara_float(blueGrp, latId, start3, count2, l1cstr->latpix_3d[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);
  status = nc_get_vara_float(blueGrp, lonId, start3, count2, l1cstr->lonpix_3d[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);
  ...
  status = nc_get_vara_float(blueGrp, latId, start3, count2, l1cstr->latpix2_3d[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);
  status = nc_get_vara_float(blueGrp, lonId, start3, count2, l1cstr->lonpix2_3d[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);


  status = nc_get_vara_float(blueGrp, IntId, start3, count2, l1cstr->I[0]);//these are 2-D arrays------------ #bands x #pixels
  check_err(status, __LINE__, __FILE__);
...
int32_t l1c_str::readl1b_misr_l1c(l1c_str *l1cstr,l1c_filehandle *l1cfile,filehandle *l1file,int32_t sline){
  size_t oneline;
  size_t start3[] = {0,0,0};
  size_t count2[] = {1,1,1};
  string str;
...
int32_t l1c_str::closel1b_spex_l1c(l1c_str *l1cstr,l1c_filehandle *l1cfile){
     ...
     if (l1cstr->viewport != nullptr)
       delete [] (l1cstr->viewport);
     if (l1cstr->senazpix != nullptr)
       delete [] (l1cstr->senazpix);
     if (l1cstr->latpix != nullptr)
       delete [] (l1cstr->latpix);
     if (l1cstr->lonpix != nullptr)
        delete [] (l1cstr->lonpix);
     if (l1cstr->latpix2 != nullptr)
       delete [] (l1cstr->latpix2);
     if (l1cstr->lonpix2 != nullptr)
        delete [] (l1cstr->lonpix2);


    if(I_lambdas) free(I_lambdas);
    if(pol_lambdas) free(pol_lambdas);

    if (l1cstr->I) free2d_float(l1cstr->I);
    if (l1cstr->I_polsample) free2d_float(l1cstr->I_polsample);
...
int32_t l1c_str::closel1b_ocis_l1c(l1c_str *l1cstr,l1c_filehandle *l1cfile){
    ...
    // From openl1b_ocis
     ...
     if (l1cstr->timepix != nullptr)
        delete [] (l1cstr->timepix);


    if (tmpShort) free(tmpShort);
    if (scan_time) free(scan_time);
    if(blue_lambdas) free(blue_lambdas);
    if(red_lambdas) free(red_lambdas);
    if(SWIR_lambdas) free(SWIR_lambdas);

    if (l1cstr->Lt_blue) free2d_float(l1cstr->Lt_blue);
    if (l1cstr->Lt_red) free2d_float(l1cstr->Lt_red);
    if (l1cstr->Lt_SWIR) free2d_float(l1cstr->Lt_SWIR);
...
int32_t l1c_str::closel1b_harp2_l1c(l1c_str *l1cstr,l1c_filehandle *l1cfile){
    ...
    if (l1cstr->senazpix_3d) free2d_float(l1cstr->senazpix_3d);
    if (l1cstr->latpix_3d) free2d_float(l1cstr->latpix_3d);
    if (l1cstr->lonpix_3d) free2d_float(l1cstr->lonpix_3d);
    if (l1cstr->latpix2_3d) free2d_float(l1cstr->latpix2_3d);
    if (l1cstr->lonpix2_3d) free2d_float(l1cstr->lonpix2_3d);
