...
int main (int argc, char* argv[]) {
  ...
  NcGroup gidGEO;
  //  cout << "Opening geolocation group" << endl;
  gidGEO = L1Cfile->getGroup( "geolocation_data");
  ...
  int nl1c = bins_along_track * bins_across_track;
  ...
  string ALBEDOfields[albedo_nfields]
  ...
  // Use only 250 lines from trailing/following datasets
  uint32_t totlines = nlines[1];
  if (nlines[0] != 0) totlines += 250;
  if (nlines[2] != 0) totlines += 250;
  ...
  /*
  // Correct lonCM is dateline crossed
  for (size_t i=0; i<totlines; i++)
    for (size_t j=0; j<npixels; j++)
        if (datelinecross && lonCM[i][j] < 0) lonCM[i][j] += 360;
  */
  ...
  for (size_t i=0; i<totlines; i++) {
    for (size_t j=0; j<npixels; j++) {
      size_t k=i*npixels+j;

      if (brow[k] != -1)
        cout << lonCM[i][j] << " " << latCM[i][j] << " "
             << lonL1C[brow[k]][bcol[k]] << " " << latL1C[brow[k]][bcol[k]]
             << " " << (int) cld_phase[brow[k]][bcol[k]] << endl;
    }
  }   
  */
  ...
  // Read l2_flags to get LAND
  uint32_t **l2flags = new uint32_t *[totlines];
  l2flags[0] = new uint32_t[npixels*totlines];
  for (size_t i=1; i<totlines; i++) l2flags[i] = l2flags[i-1] + npixels;

  readCLD<uint32_t>( ncGrp, "l2_flags", &l2flags[0][0], npixels, nlines);

  for (size_t i=0; i<totlines; i++) {
    for (size_t j=0; j<npixels; j++) {
      cldprod[i][j] = 1.0 - ((float) (l2flags[i][j] & 2)) / 2;
    }
  }
  
  tempOut.open ("wf.bin", ios::out | ios::trunc | ios::binary);
  ...
  for (size_t i=0; i<nalong; i++) {
    for (size_t j=0; j<nacross; j++) {
      int n = nobs_ic[i][j] + nobs_wc[i][j];
      if (n != 0)
        out_l1c_2d[i][j] = (binval_ic[i][j]+binval_wc[i][j]) / n;
      else
        out_l1c_2d[i][j] = -32767;
    }
  }
  ...
  //  cgal_interp2( ncm, &latCM[0][0], &lonCM[0][0], &cldprod[0][0],
  //            nl1c, &out_l1c_2d[0][0], nnc_tag_cm);

  varout = nc_output->addVar("landmask_water_fraction", ncFloat, dims);
  varout.putVar(&out_l1c_2d[0][0]);
...
int lonlat2rowcol( uint32_t nalong, uint32_t nacross, uint32_t ncm,
  ...
  /*
  cout << "Computing CLDMSK/CLD to L1C interpolation mapping" << endl;
  int nnc_tag_cm = cgal_nnc( ncm, &latCM[0][0], &lonCM[0][0],
                             nl1c, &latL1C[0][0], &lonL1C[0][0]);
  ...
  */
  ...
  ////////// WATERMASK //////////
  ///////////////////////////////
  strcpy(wmfilename, "/accounts/jgales/ocssw/share/common/gebco_ocssw_v2020.nc");
  if (strcmp(wmfilename, "") != 0) {
    float wm_pixel_size = 360.0 / 86400;
    int wm_latrow = (90 - latmax) / wm_pixel_size;
    ...
    int n_wm_latrow = (90 - latmin) / wm_pixel_size - wm_latrow + 1;
    int n_wm_loncol = (lonmax + 180) / wm_pixel_size - wm_loncol + 1;
    ...
    for (size_t i=0; i<(size_t) n_wm_latrow; i++) {
      for (size_t j=0; j<(size_t) n_wm_loncol; j++) {
        lat_wm[k] = 90 - (i+wm_latrow)*wm_pixel_size;
        lon_wm[k] = (j+wm_loncol)*wm_pixel_size - 180;
    ...
    int nwm = n_wm_latrow * n_wm_loncol;

    cout << "Computing watermask to L1C interpolation mapping" << endl;
    int nnc_tag_wm = cgal_nnc( nwm, &lat_wm[0], &lon_wm[0],
                               nl1c, &latL1C[0][0], &lonL1C[0][0]);

    // Reading landmask
    NcFile *WMfile = new NcFile( wmfilename, NcFile::read);
    ...
    if (datelinecross) {
      /*
      float *wm0 = new float[n_wm_latrow*n_wm_loncol];
      count[1] = 8640 - wm_loncol;
      var.getVar( start, count, &wm0[0]);
      for (size_t i=0; i<(size_t) n_wm_latrow; i++)
        memcpy(&wm[i*n_wm_loncol], &wm0[i*count[1]], count[1]*sizeof(float));

      start[1] = 0;
      count[1] = n_wm_loncol + wm_loncol - 8640;
      cout << count[1] << endl;
      var.getVar( start, count, &wm0[0]);
      for (size_t i=0; i<(size_t) n_wm_latrow; i++)
        memcpy(&wm[i*n_wm_loncol+(8640-wm_loncol)],
               &wm0[i*count[1]], count[1]*sizeof(float));
      delete [] wm0;
      */
    } else {
      k=0;
      var.getVar( start, count, &wmbyte[0]);
      for (size_t i=0; i<(size_t) n_wm_latrow; i++) {
        for (size_t j=0; j<(size_t) n_wm_loncol; j++) {
          wm[k] = (float) wmbyte[k];
          k++;
    ...
    cgal_interp2( nwm, &lat_wm[0], &lon_wm[0], &wm[0],
                  nl1c, &out_l1c_2d[0][0], nnc_tag_wm);
    ...
    cout << "Writing landmask" << endl;
    varout = nc_output->addVar("landmask", ncFloat, dims);
    copyVarAtts( &var, &varout);
    varout.putVar(&out_l1c_2d[0][0]);
  
    cgal_release_tag(nnc_tag_wm);
    delete [] lat_wm;
    delete [] lon_wm;
    delete [] wm;
  }
