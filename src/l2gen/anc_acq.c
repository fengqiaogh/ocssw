    ...
    #define ANC_STAT_2T_END 4  /* 2 different anc times in end of list 
                                  = files[0] = [1] != [2] */
    ...
    enum out_nam {
        ZW, MW, PR, WV, RH, SFCP, SFCRH, SFCT, ICEFR
    };

    enum out_prof {
        TPROF, RHPROF, HPROF, QPROF, O3PROF
    };
    ...
    strcpy(lcl_fil, file);
    if ((file_olci != (char *) 0) &&
            (strcmp(upcase(lcl_fil), "OLCI_TIE_METEO") == 0)) {
        fprintf(stderr,
    ...
    if (nc_open(file, 0, &ncid) != NC_NOERR) {
        ...
        if (file_olci != (char *) 0) /* data type is OLCI */ {
            ...
            if (status == NC_NOERR) {
                ...
                if (strcmp(str_attr, "OLCI Level 1b Product, Tie-Point Meteo Data Set")
                        == 0) {
                    free(str_attr);
 ...
 *******************************************************************/ {
    ...
    anc_f_stat = -1;
    if (n_anc == 3) {
        if ((files[1] == NULL) || (files[2] == NULL) || (files[1][0] == 0)
                || (files[2][0] == 0)) anc_f_stat = ANC_STAT_CLIM;
        else {
            f12_mtch = 0;
            ...
            else if ((f12_mtch == 1) && (f23_mtch == 0)) anc_f_stat = ANC_STAT_2T_END;
 ...
 *******************************************************************/ {
    ...
    if (firstcall == 0) {
        ...
        if ((met_int = (gen_int_str *) malloc(n_met * nlvl_int * ntim_int *
                sizeof (gen_int_str))) == NULL) {
            fprintf(stderr,
    ...
    for (iprm = 0; iprm < n_met; iprm++) {
        ...
        if (anc_acq_fnd_t_interp(l_time, anc_times, ntim_int, &t_interp,
                data_ix, &wt_t1) != 0) return 1;
        ...
        for (ilon = 0; ilon < npix; ilon++) {
            ...
            if (anc_acq_eval_pt(met_int, iprm, ilvl, lat, lon, t_interp,
                    data_ix, wt_t1, ntim_int, nlvl, n_met, &val, &unc) != 0) {
                fprintf(stderr,
            ...
            switch (iprm) {
                ...
                case MW: /* the mwind comes second, so then combinations cn be done */
                    ...
                    if (input->windspeed != -2000) l1rec->ws[ilon] =
                            sqrt(pow(uwnd, 2.) + pow(vwnd, 2.));
                    if (input->windangle != -2000) l1rec->wd[ilon] = atan2f(-uwnd, -vwnd)
                        * r2d;
                    ...
                    if(l1rec->uncertainty){
                        if ((uwnd + vwnd) > 0.05 * (u_unc + v_unc)) {
                            l1rec->uncertainty->dws[ilon] = sqrt((uwnd * uwnd * u_unc * u_unc +
                                    vwnd * vwnd * v_unc * v_unc) / ws_2);
                            l1rec->uncertainty->dwd[ilon] = sqrt(vwnd * vwnd * u_unc * u_unc +
                                    uwnd * uwnd * v_unc * v_unc) / ws_2;
...
int32_t anc_acq_lin_aerosol(l1str *l1rec) {
    ...
    int32_t n_met = 12; /* # met parms finally needed */
    int32_t itim, ilon, npix, iprm, t_interp, data_ix[2];
    ...
    if (firstcall == 0) {
        ...
        /*  set up the interpolation structure array */
        if ((aer_int = (gen_int_str *) malloc(n_met * nlvl * ntim_int *
                sizeof (gen_int_str))) == NULL) {
            fprintf(stderr,
    ...
    for (iprm = 0; iprm < n_met; iprm++) {
        ...
        for (ilon = 0; ilon < npix; ilon++) {
            ...
            if (anc_acq_eval_pt(aer_int, iprm, ilvl, lat, lon, t_interp,
                    data_ix, wt_t1, ntim_int, nlvl, n_met, &val, &unc) != 0) {
                fprintf(stderr,
                        "-E- %s %d: Error interpolating to file: %s\n",
                        __FILE__, __LINE__, files[0]);
 ...
 *******************************************************************/ {
    ...
    if (firstcall == 0) {
        ...
        if ((prof_int = (gen_int_str *) malloc(n_met * nlvl * ntim_int *
                sizeof (gen_int_str))) == NULL) {
            fprintf(stderr,
                    "-E- %s %d: Unable to allocate profile interpolation array\n",
                    __FILE__, __LINE__);
    ...
    for (iprm = 0; iprm < n_met; iprm++) {
        for (ilvl = 0; ilvl < nlvl; ilvl++) {
            ...
            for (ilon = 0; ilon < npix; ilon++) {
                ...
                if (anc_acq_eval_pt(prof_int, iprm, ilvl, lat, lon, t_interp,
                        data_ix, wt_t1, ntim_int, nlvl, n_met, &val, &unc) != 0) {
                    fprintf(stderr,
...
int32_t init_anc_aerosol(l1str *l1rec) {
    ...
    if ((l1rec->anc_aerosol =
            (anc_aer_struc *) malloc(sizeof ( anc_aer_struc))) == NULL) {
        fprintf(stderr,
                "-E- %s %d: Unable to allocate additional ancillary data storage 1\n",
                __FILE__, __LINE__);
        return 1;
    }
    if (((l1rec->anc_aerosol->black_carbon_ext =
            (float *) malloc(l1rec->npix * sizeof (float))) == NULL) ||
...
/*-----------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------*/ {
    ...
    if ((l1rec->anc_add =
            (anc_struc *) malloc(sizeof ( anc_struc))) == NULL) {
        fprintf(stderr,
                "-E- %s %d: Unable to allocate additional ancillary data storage 1\n",
                __FILE__, __LINE__);
        return 1;
    }
    if (((l1rec->anc_add->prof_temp =
            (float *) malloc(nlvl * npix * sizeof (float))) == NULL) ||
 ...
 *******************************************************************/ {
    ...
    if (firstcall == 0) {
        ...
        if ((oz_int = (gen_int_str *) malloc(n_oz * nlvl_int * ntim_int *
                sizeof (gen_int_str))) == NULL) {
            fprintf(stderr,
                    "-E- %s %d: Unable to allocate met interpolation array\n",
                    __FILE__, __LINE__);
    ...
    for (iprm = 0; iprm < n_oz; iprm++) {
        ...
        if (input->ozone != -2000) {
            for (ilon = 0; ilon < npix; ilon++) {
                ...
                if (anc_acq_eval_pt(oz_int, iprm, ilvl, lat, lon, t_interp,
                        data_ix, wt_t1, ntim_int, nlvl, n_oz, &val, &unc) != 0) {
                    fprintf(stderr,
                            "-E- %s %d: Error interpolating file: %s\n",
                            __FILE__, __LINE__, files[0]);
                    return -1;
 ...
 *******************************************************************/ {
    ...
    for (iprm = 0; iprm < n_raw_gmao; iprm++) {
        /* get the GMAO array */
        if (anc_acq_read_gmao(file, ob_gmao_grp[iprm], ob_gmao_prm_nm[iprm],
                &data, &qual, &time, &nlon, &nlat, &nlvl, &lon_coord,
                &lat_coord) != 0)
            return 1;
        ...
        switch (iprm) {
            ...
            case PR:
                ...
                break;
            case RH:
                if (anc_acq_read_gmao(file, "met", "T10M", &data2, &qual2, &time,
                        &nlon, &nlat, &nlvl, &lon_coord2, &lat_coord2) != 0)
                    return 1;
                free(lat_coord2);
                free(lon_coord2);
                for (iv = 0; iv < nv; iv++)
                ...
                if (anc_acq_read_gmao(file, "met", "QV10M", &data3, &qual2, &time,
                        &nlon, &nlat, &nlvl, &lon_coord2, &lat_coord2) != 0)
                    return 1;
                free(lat_coord2);
                free(lon_coord2);
                ...
                if ((data_rh = (float *) malloc(nlon * nlat * sizeof ( float)))
                        == NULL) {
                    fprintf(stderr, "-E- %s, %d: malloc failure\n", __FILE__, __LINE__);
                    return 1;
                }
                met_cvt_q_to_rh(nv, data, MET_UNITS__P_PA, data2, MET_UNITS__T_K,
                        data3, MET_UNITS__Q_KG_KG, data_rh);
            ...
            case ICEFR:
                ...
                if (anc_acq_read_gmao(file, "lnd_ice", "FRSNO", &data2, &qual2,
                        &time, &nlon, &nlat, &nlvl, &lon_coord2, &lat_coord2) != 0)
                    return 1;
                free(lat_coord2);
                free(lon_coord2);
                for (iv = 0; iv < nv; iv++)
       ...
       -> met_int[iprm].int_id  */
        ...
        for (iv = 0; iv < nv; iv++)
            ddata[iv] = data[iv];
        if (gsl_spline2d_init(met_int[iprm].int_id, met_int[iprm].lon_coord,
                met_int[iprm].lat_coord, ddata, nlon, nlat) != 0) {
...
int32_t anc_acq_gmao_prof_prep(char *file, gen_int_str *prof_int,
        int32_t nlvl_expect)
/*******************************************************************
 ...
 *******************************************************************/ {
    ...
    for (iprm = 0; iprm < n_raw_gmao; iprm++) {
        /* get the GMAO array */
        if (anc_acq_read_gmao(file, "", ob_gmao_prm_nm[iprm], &data, &qual,
                &time, &nlon, &nlat, &nlvl, &lon_coord, &lat_coord) != 0)
            return 1;
        ...
        for (ilvl = 0; ilvl < nlvl; ilvl++) {
            ...
            prof_int[loc].int_id = gsl_spline2d_alloc(gsl_interp2d_bilinear,
                    nlon, nlat);
            ...
            if (gsl_spline2d_init(prof_int[loc].int_id, prof_int[loc].lon_coord,
                    prof_int[loc].lat_coord, ddata, nlon, nlat) != 0) {
                fprintf(stderr, "-E- %s %d: GSL 2-D initialization failed, file: %s\n",
                        __FILE__, __LINE__, file);
                return 1;
            }
 ...
 *******************************************************************/ {
    ...
    /* read the ozone */
    if (anc_acq_read_gmao(file, "met", "TO3", &data, &qual, &time, &nlon,
    ...
    for (iv = 0; iv < nv; iv++)
        ddata[iv] = data[iv];
    if (gsl_spline2d_init(oz_int[iprm].int_id, oz_int[iprm].lon_coord,
            oz_int[iprm].lat_coord, ddata, nlon, nlat) != 0) {
        fprintf(stderr, "-E- %s %d: GSL 2-D initialization failed, file: %s\n",
                __FILE__, __LINE__, file);
        return 1;
    }
...
int32_t anc_acq_gmao_aer_prep(char *file, gen_int_str *aer_int) {
    ...
    char *ob_gmao_prm_nm[] = {"BCEXTTAU", "BCSCATAU", "DUEXTTAU", "DUSCATAU",
        "SSEXTTAU", "SSSCATAU", "SUEXTTAU", "SUSCATAU", "OCEXTTAU", "OCSCATAU",
        "TOTEXTTAU", "TOTSCATAU"};

    int32_t n_raw_gmao = 12;
    ...
    for (iprm = 0; iprm < n_raw_gmao; iprm++) {
        ...
        for (iv = 0; iv < nv; iv++)
            ddata[iv] = data[iv];
        if (gsl_spline2d_init(aer_int[iprm].int_id, aer_int[iprm].lon_coord,
                aer_int[iprm].lat_coord, ddata, nlon, nlat) != 0) {
            fprintf(stderr, "-E- %s %d: GSL 2-D initialization failed, file: %s\n",
                    __FILE__, __LINE__, file);
            return 1;
        }
...
int32_t anc_acq_eval_pt(gen_int_str *met_int, int32_t iprm, int32_t ilvl,
        float lat, float lon, int32_t t_interp, int32_t *data_ix, float wt_t1,
        int32_t ntim_int, int32_t nlvl, int32_t nprm, float *final_val, float *unc)
/*******************************************************************
 ...
 *******************************************************************/ {
    ...
    *unc = 0.;
    for (itim = 0; itim < ntim; itim++) {
        ...
        if (gsl_spline2d_eval_e(met_int[met_ptr].int_id, lon, lat, accel_lon,
                accel_lat, &val) != 0) {
            fprintf(stderr, "-E- %s %d: gsl_spline2d_eval_e error: %d, %d, %d\n",
                    __FILE__, __LINE__, itim, iprm, ilvl);
            return -1;
        }
        /*  make sure the value is OK and make bad if not */
        ilat = gsl_interp_accel_find(accel_lat,
                met_int[met_ptr].lat_coord, nlat, lat);
        ilon = gsl_interp_accel_find(accel_lon,
                met_int[met_ptr].lon_coord, nlon, lon);
    ...
    if (*final_val == BAD_FLT) *unc = BAD_FLT;
    return 0;
}

int32_t anc_acq_fnd_t_interp(double s_time, double *anc_time,
        int32_t anc_f_stat, int32_t *t_interp, int32_t *data_ix, float *wt)
/*******************************************************************
 ...
 *******************************************************************/ {
    ...
    if (((*nlat = ncio_dim_siz(ncid, "lat")) == -1) ||
            ((nlon_pre = ncio_dim_siz(ncid, "lon")) == -1)) {
        fprintf(stderr,
    ...
    *time = isodate2unix((const char*) isodate);
    ...
    if ((nc_inq_varid(ncid, ds_name, &var_id) != NC_NOERR) ||
        ...
        fprintf(stderr,
                ...
                __FILE__, __LINE__, file, ds_name);
        return 1;
    }
    ...
    for (ilvl = 0; ilvl < *nlvl; ilvl++) {
        for (il = 0; il < *nlat; il++) {
            for (ip = 0; ip < *nlon; ip++) {
                ...
                if (ip < nlon_pre) {
                    *(*data + loc) =
                            *(data_tmp + ip + nlon_pre * (il + *nlat * ilvl));
                } else {
...
int32_t anc_acq_ecmwf_init(char **files, char **prm_nm, int n_prm,
        int32_t sto_ix)
/*******************************************************************
 ...
 *******************************************************************/ {
    ...
    for (ids = 0; ids < 3; ids++) {
         ...
         */
        if ((ids > 0) && (anc_f_stat == ANC_STAT_1T)) break;
        if ((ids == 1) && (f12_mtch == 1)) continue;
        if ((ids == 2) && (f23_mtch == 1)) break;
 ...
 *******************************************************************/ {
   ...
   */
    ...
    char *anc_prm_nm[] = {"humidity", "horizontal_wind", "sea_level_pressure",
    ...
    if (firstcalls[anc_class]) {
        ...
        if (nc_get_att_int(ncid[anc_class], NC_GLOBAL, "ac_subsampling_factor",
                &pix_smp[anc_class]) != NC_NOERR) {
            fprintf(stderr,
                    "-E- %s %d: Unable to read column sampling attrib from OLCI tie point anc file: %s\n",
                    __FILE__, __LINE__, file);
            return -1;
        }
        if (nc_get_att_int(ncid[anc_class], NC_GLOBAL, "al_subsampling_factor",
                &lin_smp) != NC_NOERR) {
            fprintf(stderr,
        ...
        if (nc_inq_dimlen(ncid[anc_class], nid, &tie_npix[anc_class])
                != NC_NOERR) {
            fprintf(stderr,
                    "-E- %s %d: Unable to get OLCI tie point meteo # pixels\n",
                    __FILE__, __LINE__);
        ...
        if (tie_epix < epix) {
            fprintf(stderr,
                    ...
                    __FILE__, __LINE__, tie_epix, epix);
            fprintf(stderr,
                    "tie point sampling: %d and # pixels: %d\n",
                    pix_smp[anc_class], (int) tie_npix[anc_class]);
            return -1;
        }
        /*
         ...
         */
        if ((tie_data = (float *) malloc(tie_npix[anc_class] * sizeof (float)))
                == NULL) {
            fprintf(stderr,
        ...
        if (anc_class == 0) tie_met = tie_data;
        else tie_oz = tie_data;

        if ((qual = (int32_t *) malloc(tie_npix[anc_class] * sizeof (int32_t)))
                == NULL) {
            fprintf(stderr,
        ...
        for (ids = 0; ids < n_ds_prm; ids++) {
            ...
            if (nc_inq_varid(ncid[anc_class], anc_prm_nm[ptr_prm], &varid[ptr_prm])
                    != NC_NOERR) {
                fprintf(stderr,
            ...
            if (nc_get_att_float(ncid[anc_class], varid[ptr_prm], "_FillValue",
                    &fill[ptr_prm]) != NC_NOERR) {
                fprintf(stderr,
            ...
            if (nc_get_att_float(ncid[anc_class], varid[ptr_prm], "valid_min",
                    &valid_min[ptr_prm]) != NC_NOERR) {
                fprintf(stderr,
                        "-E- %s %d: Can't get valid_min for OLCI anc tie point dataset: %s\n",
                        __FILE__, __LINE__, anc_prm_nm[ptr_prm]);
                return -1;
            }
            if (nc_get_att_float(ncid[anc_class], varid[ptr_prm], "valid_max",
                    &valid_max[ptr_prm]) != NC_NOERR) {
                fprintf(stderr,
    ...
    for (ids = 0; ids < n_ds_prm; ids++) {
        ...
        for (ifld = 0; ifld < n_field_per_parm; ifld++) {
            ...
            if (iscan < tie_nlin[anc_class]) {
                /*  read the line  */
                if ((stat = nc_get_vara_float(ncid[anc_class], varid[ptr_prm],
                        start, count, tie_data)) != NC_NOERR) {
                    fprintf(stderr,
                ...
                for (ipx = 0; ipx < tie_npix[anc_class]; ipx++) {
                    if ((*(tie_data + ipx) == fill[ptr_prm]) ||
                            (*(tie_data + ipx) < valid_min[ptr_prm]) ||
                            (*(tie_data + ipx) > valid_max[ptr_prm]))
                        *(qual + ipx) = 1;
                    else {
                        *(qual + ipx) = 0;
                        if (ptr_prm == 3) *(tie_data + ipx) *= 0.1; /* for pw conversion
             kg m-2 -> g cm-2 */
                        if (ptr_prm == 4) *(tie_data + ipx) *= 46.698; /* for oz
                ...
                for (ipx = 0; ipx < npix; ipx++) {
                    ...
                    if ((*(qual + px_tie1) == 1) || (*(qual + px_tie2) == 1)) {
                        ...
                        *(ar + ipx) = anc_miss_fill(ptr_prm);
                        ...
                        /*  interpolate */
                        fr_dist = (float) (ipx_dat - px_tie1 * pix_smp[anc_class]) /
                                (float) pix_smp[anc_class];
                        *(ar + ipx) = tie_data[px_tie1] * (1. - fr_dist) +
                                tie_data[px_tie2] * fr_dist;
                    }
                }
            } else {
