...
aermodstr* alloc_aermodstr(int nbands, int nscatt, int nphi, int nsolz, int nsenz, int ntheta, int npc, int ntau_870) {
    ...
    if(use_pca_lut) {
        ...
        model->pc_td = allocate3d_float(ntau_870, nsolz, npc);
        ...
        // pc(pc)
        model->pc = (short*) malloc(npc * sizeof(short));                  
    } else {
        if(nscatt > 0) {
...
int load_aermod(int32_t sensorID, float wave[], int32_t nwave, char *aermodfile, char models[MAXAERMOD][32], int32_t nmodels) {
    ...
    for (im = 0; im < nmodels + 1; im++) {
        ...
        if(use_netcdf) {
            ...
            read_dimension_size(file, nc_id, "wave", &aer_nwave);
            read_dimension_size(file, nc_id, "solz", &nsolz);
            read_dimension_size(file, nc_id, "senz", &nsenz);
            read_dimension_size(file, nc_id, "phi", &nphi);
            if(use_pca_lut) {
                read_dimension_size(file, nc_id, "pc", &npc);
                read_dimension_size(file, nc_id, "tau_870", &ntau_870);                
            } else {
                read_dimension_size(file, nc_id, "dtran_wave", &dtran_nwave);
                read_dimension_size(file, nc_id, "dtran_theta", &dtran_ntheta);
            }
         } else {
        ...
        if(use_netcdf) {
            int fmf;
            status = nc_get_att_int(nc_id, NC_GLOBAL, "AerosolFMF", &fmf);
            if(status != NC_NOERR) {
        ...
        if(use_pca_lut) {
            read_lut_variable(file, nc_id, "pc_rhoa", aertab->model[im]->pc_rhoa[0][0][0][0]);
            read_lut_variable(file, nc_id, "pc_components_rhoa", aertab->model[im]->pc_components_rhoa[0]);
            read_lut_variable(file, nc_id, "pc_mean_rhoa", aertab->model[im]->pc_mean_rhoa);
            read_lut_variable(file, nc_id, "pc_td", aertab->model[im]->pc_td[0][0]);
            read_lut_variable(file, nc_id, "pc_components_td", aertab->model[im]->pc_components_td[0]);
            read_lut_variable(file, nc_id, "pc_mean_td", aertab->model[im]->pc_mean_td);
            read_lut_variable(file, nc_id, "angstrom", aertab->model[im]->angstrom);
            read_lut_variable_short(file, nc_id, "pc", aertab->model[im]->pc);
            read_lut_variable(file, nc_id, "tau_870", aertab->model[im]->tau_870);                
        } else {
            if(!use_netcdf) {
        ...
        float tau_iwnir_l, int32_t modl, float tau_pred[], float rho_pred[],float derv_rhoa[], float derv_taua_tmp[])
 {
  ...
  /*  if(tau_iwnir_l<aermod->tau_870[0]|| tau_iwnir_l>aermod->tau_870[ntau_870-1]){
    ...
    for(iw=0;iw<nwave;iw++){
        ...
        rhoa1=exp(rhoa1+aermod->pc_mean_rhoa[iw]);
        rhoa2=exp(rhoa2+aermod->pc_mean_rhoa[iw]);
...
int ahmad_atm_corr_pca(int32_t sensorID, float wave[], int32_t nwave, int32_t iwnir_s, int32_t iwnir_l,
    ...
    for (im = 0; im < nmodels; im++) {
        ...
        for(itau=0;itau<ntau_870;itau++){
            ...
            rhoa_modl_s[itau]=exp(rhoa_modl_s[itau]);
            rhoa_modl_l[itau]=exp(rhoa_modl_l[itau]);
        }
    ...
    for (iw = 0; iw < nwave; iw++) {
        ...
        if(uncertainty){
            for(inir=iwnir_s;inir<=iwnir_l;inir++){
                ...
                if(inir==iwnir_l){
                    uncertainty->derv_tsol_rhorc[iw][inir-iwnir_s]+=(1.0 - modrat)*dtmin[iw]*uncertainty->derv_taua_min_rhorc_l[iwnir_l];
                    uncertainty->derv_tsol_rhorc[iw][inir-iwnir_s]+=  modrat      *dtmax[iw]*uncertainty->derv_taua_max_rhorc_l[iwnir_l];
                }
            }
    ...
    for (iw = 0; iw < nwave; iw++) {
        ...
        if(uncertainty){
            for(inir=iwnir_s;inir<=iwnir_l;inir++){
                ...
                if(inir==iwnir_l){
                    uncertainty->derv_tsen_rhorc[iw][inir-iwnir_s]+=(1.0 - modrat)*dtmin[iw]*uncertainty->derv_taua_min_rhorc_l[iwnir_l];
                    uncertainty->derv_tsen_rhorc[iw][inir-iwnir_s]+=  modrat      *dtmax[iw]*uncertainty->derv_taua_max_rhorc_l[iwnir_l];
                }
            }
...
int smaer_pca(int32_t sensorID, float wave[], int32_t nwave, int32_t iwnir_s,
    ...
    float *derv_taua_rhorc_l;//[nmodels], derivative of modeled taua[nir_l] to rhorc_l
    ...
    if(uncertainty){
        for(iw = iwnir_s; iw <= iwnir_l; iw++)
            derv_chi_obs_rhorc[iw-iwnir_s]=0.;
    ...
    for (iw = iwnir_s; iw <= iwnir_l; iw++) {
        ...
        mbac_wsum += mbac_w[iw];
        if(uncertainty){
            derv_chi_obs_rhorc[iw-iwnir_s]=1/rhoa[nir_l];
            derv_chi_obs_rhorc[nir_l-iwnir_s]+=(-rhoa[iw]/rhoa[nir_l]/rhoa[nir_l]);
    ...
    if(uncertainty){
        ...
        for(iw = iwnir_s; iw <= iwnir_l; iw++)
            derv_chi_obs_rhorc[iw-iwnir_s]/=mbac_wsum;
    }
    ...
    for (im = 0; im < nmodels; im++) {
        ...
        if(uncertainty){
            derv_chi_rhorc[im][nir_l-iwnir_s]=0.;
        ...
        for (iw = iwnir_s; iw <= iwnir_l; iw++) {
            ...
            if(uncertainty){
                ...
                derv_chi_rhorc [im][nir_l-iwnir_s]+=derv_temp_rhorc;
        ...
        if(uncertainty){
            derv_chi_rhorc[im][nir_l-iwnir_s]/=mbac_wsum;
     ...
     *modmax=*modmin;
        ...
        *modrat=(chi_obs-chi_struct[0].value)/(chi_struct[1].value-chi_struct[0].value);
        if(uncertainty){
            ...
            for(iw=iwnir_s;iw<iwnir_l;iw++){
                if(iw!=nir_l){
                    derv_modrat_rhorc[iw-iwnir_s]=derv_chi_obs_rhorc[iw-iwnir_s]/(chi_struct[1].value-chi_struct[0].value);
                    derv_modrat_rhow_l+=-derv_modrat_rhorc[iw-iwnir_s]*uncertainty->ratio_rhow[iw-iwnir_s];
                }
            }

            derv_modrat_rhorc[nir_l-iwnir_s]=derv_modrat_chi0*derv_chi_rhorc[*modmin][nir_l-iwnir_s]+derv_modrat_chi1*derv_chi_rhorc[*modmax][nir_l-iwnir_s];
            derv_modrat_rhorc[nir_l-iwnir_s]+=derv_chi_obs_rhorc[nir_l-iwnir_s]/(chi_struct[1].value-chi_struct[0].value);
            derv_modrat_rhow_l+=-derv_modrat_rhorc[nir_l-iwnir_s];
       ...
       for (iw = 0; iw <nwave; iw++) {
           ...
           if(uncertainty){
               for(i=iwnir_s;i<iwnir_l;i++){
                   if(i!=nir_l){
                       uncertainty->derv_La_rhorc[iw][i-iwnir_s]=(rho_pred_max[iw]-rho_pred_min[iw])*derv_modrat_rhorc[i-iwnir_s];
                       uncertainty->derv_La_rhow_l[iw]+=-uncertainty->derv_La_rhorc[iw][i-iwnir_s]*uncertainty->ratio_rhow[i-iwnir_s];
                   }
               }
               uncertainty->derv_La_rhorc[iw][nir_l-iwnir_s]=(rho_pred_max[iw]-rho_pred_min[iw])*derv_modrat_rhorc[nir_l-iwnir_s];
               uncertainty->derv_La_rhorc[iw][iwnir_l-iwnir_s]+= (1-*modrat)*derv_rhoa_min[iw]*derv_taua_rhorc_l[*modmin];
               uncertainty->derv_La_rhorc[iw][iwnir_l-iwnir_s]+= (*modrat)  *derv_rhoa_max[iw]*derv_taua_rhorc_l[*modmax];
               uncertainty->derv_La_rhow_l[iw]+=-uncertainty->derv_La_rhorc[iw][nir_l-iwnir_s];
