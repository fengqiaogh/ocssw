...
int atmocor2(l2str *l2rec, aestr *aerec, int32_t ip) {
    ...
    if (firstCall == 1) {
        ...
        case AERRHSM:
            ...
            daer=1;
            nbands_mbac=0;
            while(input->mbac_wave[nbands_mbac]!=-1)
                nbands_mbac++;
            if(nbands_mbac<2){
                nbands_mbac=2;
                input->mbac_wave[0]=input->aer_wave_short;
                input->mbac_wave[1]=input->aer_wave_long;
            }
...
NIRSWIR:
    ...
    if (aer_opt==AERRHSM ) {//&& tLw_nir[aer_s]>0.
        for (ib = aer_s; ib <=  aer_l; ib ++) {
            for(int i=0;i<nbands_mbac;i++)
                if(l1file->iwave[ib]==input->mbac_wave[i])
                    mbac_w[ib]=1.;
        }
    }
        ...
        if (status == 0) {
            ...
            for (ib = 0; ib < nwave; ib++) {
                ...
                if(uncertainty){
                    ...
                    for(inir=aer_s;inir<=aer_l;inir++)
                        dt_sen[ib] += pow(uncertainty->derv_tsen_rhorc[ib][inir-aer_s] * sqrt(dLtemp[inir]) * PI/(mu0 * Fo[inir]), 2);

                    dt_sen[ib] += pow(uncertainty->derv_tsen_rhow_l[ib] * dtLw_nir[aer_l] * PI/ (mu0 * Fo[aer_l]), 2);
                    dt_sen[ib] += pow(uncertainty->derv_tsen_rh[ib] * uncertainty->drh[ip],2);

                    /* vicarious calibration contribution */
                    for(inir=aer_s;inir<=aer_l;inir++)
                        dt_sen[ib] += pow(uncertainty->derv_tsen_rhorc[ib][inir-aer_s] * PI / (mu0 * Fo[inir])* dvc[inir], 2);

                    for(inir=aer_s;inir<aer_l;inir++)
                        dt_sen[ib]+=2*corr_nir_l[inir]*(uncertainty->derv_tsen_rhorc[ib][inir-aer_s]*PI/(mu0*Fo[inir])*dvc[inir]*uncertainty->derv_tsen_rhorc[ib][aer_l-aer_s]*PI/(mu0*Fo[aer_l])*dvc[aer_l]);
                    ...
                    for(inir=aer_s;inir<=aer_l;inir++)
                        dt_sol[ib] += pow(uncertainty->derv_tsol_rhorc[ib][inir-aer_s] * sqrt(dLtemp[inir]) * PI/ (mu0 * Fo[inir]), 2);

                    dt_sol[ib] += pow(uncertainty->derv_tsol_rhow_l[ib] * dtLw_nir[aer_l] * PI/ (mu0 * Fo[aer_l]), 2);
                    dt_sol[ib] += pow(uncertainty->derv_tsol_rh[ib] * uncertainty->drh[ip], 2);

                    /* vicarious calibration contribution */
                    for(inir=aer_s;inir<=aer_l;inir++)
                        dt_sol[ib] += pow(uncertainty->derv_tsol_rhorc[ib][inir-aer_s] * PI / (mu0 * Fo[inir])* dvc[inir], 2);

                    for(inir=aer_s;inir<aer_l;inir++)
                        dt_sol[ib]+=2*corr_nir_l[inir]*(uncertainty->derv_tsol_rhorc[ib][inir-aer_s]*PI/(mu0*Fo[inir])*dvc[inir]*uncertainty->derv_tsol_rhorc[ib][aer_l-aer_s]*PI/(mu0*Fo[aer_l])*dvc[aer_l]);
                    ...
                    for(inir=0;inir<=aer_l-aer_s;inir++){
                    ...
                    for(inir=0;inir<=aer_l-aer_s;inir++)
                        nLw_unc[ib] +=pow(derv_rhorc[inir] * sqrt(dLtemp[inir+aer_s]) * PI/ (mu0 * Fo[inir+aer_s]), 2);

                    nLw_unc[ib] += pow(derv_rhow_l * dtLw_nir[aer_l] * PI / (mu0 * Fo[aer_l]),2);
                    ...
                    for(inir=0;inir<=aer_l-aer_s;inir++)
                        dvc_temp += pow(derv_rhorc[inir] * dvc[inir+aer_s] * PI / (mu0 * Fo[inir+aer_s]), 2);

                    dvc_temp+=2*corr_nir_l[ib]*(derv_rhorc[aer_l-aer_s]*dvc[aer_l]*PI/(mu0*Fo[aer_l])*dvc[ib]/t_sol[ib]/t_sen[ib]);
                    dvc_temp+=2*corr_nir_s[ib]*(dvc[ib]/t_sol[ib]/t_sen[ib]*derv_rhorc[aer_s-aer_s]*dvc[aer_s]*PI/(mu0*Fo[aer_s]));
                    dvc_temp+=2*corr_nir_s[aer_l]*(derv_rhorc[aer_l-aer_s]*dvc[aer_l]*PI/(mu0*Fo[aer_l])*derv_rhorc[aer_s-aer_s]*dvc[aer_s]*PI/(mu0*Fo[aer_s]));
                    ...
                    for(inir=0;inir<=aer_l-aer_s;inir++)
                        dtaua[ib] += pow(uncertainty->derv_taua_rhorc[ib][inir] * sqrt(dLtemp[inir+aer_s]) * PI/ (mu0 * Fo[aer_l]), 2);

                    dtaua[ib] += pow(uncertainty->derv_taua_rhow_l[ib] * dtLw_nir[aer_l] * PI/ (mu0 * Fo[aer_l]), 2);
                    dtaua[ib] += pow(uncertainty->derv_taua_rh[ib] * uncertainty->drh[ip], 2);

                    /* vicarious calibration contribution */
                    for(inir=0;inir<=aer_l-aer_s;inir++)
                        dtaua[ib] += pow(uncertainty->derv_taua_rhorc[ib][inir] * PI / (mu0 * Fo[inir+aer_s])* dvc[inir+aer_s], 2);
                    dtaua[ib]+=2*corr_nir_s[aer_l]*(uncertainty->derv_taua_rhorc[ib][aer_l-aer_s]*PI/(mu0*Fo[aer_l])*dvc[aer_l]*uncertainty->derv_taua_rhorc[ib][aer_s-aer_s]*PI/(mu0*Fo[aer_s])*dvc[aer_s]);
                    /*                                    */
                     if(dtaua[ib]<0)
                         dtaua[ib]=0;
