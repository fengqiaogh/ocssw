...
int lonlat2pixline4(char *l1c_filename, float lon, float lat,
        short *row, short *col) {
    lonlat2pixline_t params;
    file_type type;
    l1str *l1rec = nullptr;
    size_t xbins,ybins;
    float **lat_gd=nullptr,**lon_gd=nullptr;
    int flag_out=-1;
    ...
    strcpy(params.input_filename, l1c_filename);
    ...
    if(type==FT_L1C){
//open grid
      open_l1c(l1c_filename,&ybins,&xbins,lat_gd,lon_gd);
//      printf("%s%d%s%d","ybins..",ybins,"xbins..",xbins);
//      search_l1c(l1file,l1rec,ybins,xbins,lat_gd,lon_gd,gdindex);
    flag_out=search2_l1c(ybins,xbins,lat,lon,lat_gd,lon_gd,row,col);
    //if inside the L1C grid----
    }
...
int lonlat2pixline5(char *l1c_filename,float SWlon, float SWlat, float NElon, float NElat,
        short *scol, short *ecol, short *srow, short *erow) {
    lonlat2pixline_t params;
    file_type type;
    l1str *l1rec = nullptr;
    size_t xbins,ybins;
    float **lat_gd=nullptr,**lon_gd=nullptr;
    int flag_out=-1,flag_out1=-1,flag_out2=-1;
...
//open grid
      ...
      flag_out1=search2_l1c(ybins,xbins,SWlat,SWlon,lat_gd,lon_gd,srow,scol);
      flag_out2=search2_l1c(ybins,xbins,NElat,NElon,lat_gd,lon_gd,erow,ecol);
//      printf("so far so good.%s%d%s%d.","ybins..",ybins,"xbins..",xbins);        
      printf("so far so good.%s%d%s%d.","flag_out1..",flag_out1,"flag_out2..",flag_out2);      

       if (flag_out1==0 && flag_out2==0) {
         flag_out=0;
        }
   }
