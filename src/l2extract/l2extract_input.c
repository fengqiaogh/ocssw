...
int l2extract_init_options(clo_optionList_t* list, const char* softwareVersion) {
    ...
    clo_setHelpStr("Usage: l2extract argument-list"
    ...
    clo_addOption(list, "product", CLO_TYPE_STRING, NULL, "comma separated list of products, empty string outputs\n        all products");
...
int l2extract_read_options(clo_optionList_t* list, int argc, char* argv[]) {
    ...
    if (clo_isOptionSet(option)) {
        ...
        // sub-sensor suite
        if (subsensorId != -1) {
            sprintf(tmpStr, "%s/%s/%s/l2extract_defaults_%s.par", dataRoot,
                    sensorDir, subsensorId2SubsensorDir(subsensorId), suiteStr);
            if (access(tmpStr, R_OK) != -1) {
                if (want_verbose)
                    printf("Loading default parameters from %s\n", tmpStr);
                clo_readFile(list, tmpStr);
