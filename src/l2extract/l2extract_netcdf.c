...
#define   GEOBOX_INC 20.0
    ...
    if(reflectanceLocationValuesFound) {
        NCDIE(nc_def_dim(rootGroup_w, "number_of_reflectance_location_values", numReflectanceLocationValues, &numReflectiveLocationValuesDimId));
    }
    ...
    if (status == NC_NOERR) {
        ...
        for (i = 0; i < numVariables; i++) {
            ...
            if (checkIfInProdlist(prodlist, name) == 0) continue;
            copyVariable(group_r, name, start, count, dimIds, group_w, NULL);
        }
    }
