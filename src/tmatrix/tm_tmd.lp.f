...
C   and must be used along with the accompanying files tm_tmd.par.f
C   and lpd.f.    
...
C   value specified in the PARAMETER statement in the file tm_tmd.par.f.
...
C   tm_tmd.par.f.  It is recommended to keep NPN1=NPN4+25 and
...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
     ...
     &     REFF,VEFF,CEXTIN,CSCAT,WALB,ASYMM, ALPHA, BETA, FM, SZ)
      IMPLICIT REAL*8 (A-H,O-Z)
      INCLUDE 'tm_tmd.par.f'
      REAL*8 REFF(NPNAX),VEFF(NPNAX),CEXTIN(NPNAX)
      REAL*8 CSCAT(NPNAX),WALB(NPNAX),ASYMM(NPNAX)
      REAL*8 ALPHA(4,NPL,NPNAX),BETA(2,NPL,NPNAX),FM(6,NPNA,NPNAX),SZ(NPNAX)
Cf2py intent(in) rat
Cf2py intent(in) lam
Cf2py intent(in) mrr
Cf2py intent(in) mri
...
C     PRINT 8000, RAT
 ...
 8002    FORMAT('R1=',F10.6,'   R2=',F10.6)
         IF (ABS(RAT-1D0).LE.1D-6) PRINT 8003, REFF(IAX),VEFF(IAX)
         IF (ABS(RAT-1D0).GT.1D-6) PRINT 8004, REFF(IAX),VEFF(IAX)
     ...
     &          'IN SIZE AVERAGING =',I4)
         ...
         DO 500 INK=1,NK
            ...
            IF (INM1.GE.NPN1) THEN
                ...
                GOTO 600
            ENDIF
 7333 FORMAT('INM1 (',I3,') is greater than NPN1 (',I3,') for radius ',D8.2)
            QEXT1=0D0
            QSCA1=0D0
            DO 50 NMA=INM1,NPN1
               NMAX=NMA
               ...
               IF (NGAUSS.GT.NPNG1) THEN
                    ...
                    GOTO 600
                ENDIF
 7340 FORMAT('NGAUSS (',I3,') is greater than NPNG1 (',I3,') for radius ',D8.2)
...
C               PRINT 7335, NMAX1,DQSCA,DQEXT
   ...
   55       NNNGGG=NGAUSS+1
            IF (NGAUSS.GE.NPNG1) THEN
                PRINT 7336, NGAUSS, NPNG1, A
                PRINT 7337, NG, DSCA, DEXT
                REFF(IAX)=-999.9
                VEFF(IAX)=-999.9
                CEXTIN(IAX)=-999.9
                CSCAT(IAX)=-999.9
            ...
            DO 150 NGAUS=NNNGGG,NPNG1
               NGAUSS=NGAUS
               NGGG=2*NGAUSS
 7336  FORMAT('NGAUSS (',I3,') is greater than NPNG1 (',I3,') for size ',D8.2)
...
C               PRINT 7337, NGGG,DSCA,DEXT
               QEXT1=QEXT
               QSCA1=QSCA
               IF(DSCA.LE.DDELT.AND.DEXT.LE.DDELT) GOTO 155
               IF (NGAUS.GE.NPNG1) THEN
                    PRINT 7336, NGAUS, NPNG1, A
  ...
  155       CONTINUE
            ...
            DO 204 N=1,NNM
               QEXT=QEXT+TR1(N,N)
  204       CONTINUE
            IF (NMAX1.GE.NPN4) THEN
                PRINT 7550, NMAX1, NPN4, A
                REFF(IAX)=-999.9
                VEFF(IAX)=-999.9
                CEXTIN(IAX)=-999.9
                CSCAT(IAX)=-999.9
                ...
                GOTO 600
            ENDIF
 7550 FORMAT ('NMAX1 (',I3,') is greater than NPN4 (',I3,') for radius ',D8.2)
            IF (NMAX1.GT.NPN4) RETURN
            DO 213 N2=1,NMAX1
               NN2=N2+NMAX
               DO 213 N1=1,NMAX1
                  NN1=N1+NMAX
...
C            PRINT 7880, NMAX,NMAX1
 7880       FORMAT ('nmax=',I3,'   nmax1=',I3)
  ...
  250       CONTINUE
            CSCAT(IAX)=CSCAT(IAX)+WGI
            CEXTIN(IAX)=CEXTIN(IAX)+CEXT*WGII
C            PRINT 6070, I,NMAX,NMAX1,NGAUSS
 6070       FORMAT(4I6)
  500    CONTINUE
         DO 510 L1=1,L1MAX
            ALPH1(L1)=ALPH1(L1)/CSCAT(IAX)
            ALPH2(L1)=ALPH2(L1)/CSCAT(IAX)
            ALPH3(L1)=ALPH3(L1)/CSCAT(IAX)
            ALPH4(L1)=ALPH4(L1)/CSCAT(IAX)
            BET1(L1)=BET1(L1)/CSCAT(IAX)
            BET2(L1)=BET2(L1)/CSCAT(IAX)
            ALPHA(1,L1,IAX)=ALPH1(L1)
            ALPHA(2,L1,IAX)=ALPH2(L1)
            ALPHA(3,L1,IAX)=ALPH3(L1)
            ALPHA(4,L1,IAX)=ALPH4(L1)
            BETA(1,L1,IAX)=BET1(L1)
            BETA(2,L1,IAX)=BET2(L1)
  510    CONTINUE
         WALB(IAX)=CSCAT(IAX)/CEXTIN(IAX)
         CALL HOVENR(L1MAX,ALPH1,ALPH2,ALPH3,ALPH4,BET1,BET2)
         ASYMM(IAX)=ALPH1(2)/3D0
         PRINT 9100,CEXTIN(IAX),CSCAT(IAX),WALB(IAX),ASYMM(IAX)
 9100    FORMAT('CEXT=',D12.6,2X,'CSCA=',D12.6,2X,
     &          2X,'W=',D12.6,2X,'<COS>=',D12.6)
         IF (WALB(IAX).GT.1D0) PRINT 9111
 9111    FORMAT ('WARNING: W IS GREATER THAN 1')
         WRITE (10,580) WALB(IAX),L1MAX
         DO L=1,L1MAX
  ...
  580    FORMAT(D14.8,I8)
         LMAX=L1MAX-1
         CALL MATR (ALPH1,ALPH2,ALPH3,ALPH4,BET1,BET2,LMAX,NPNA,NPNAX,IAX,FM)
  600 CONTINUE
      ITIME=MCLOCK()
...
C        180*(I-1)/(N-1) (DEGREES), WHERE I NUMBERS THE ANGLES
      ...
      SUBROUTINE MATR(A1,A2,A3,A4,B1,B2,LMAX,NPNA,NPNAX,IAX,FM)
      INCLUDE 'tm_tmd.par.f'
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8 A1(NPL),A2(NPL),A3(NPL),A4(NPL),B1(NPL),B2(NPL),FM(6,NPNA,NPNAX)
      N=NPNA
      DN=1D0/DFLOAT(N-1)
      DA=DACOS(-1D0)*DN
      DB=180D0*DN
