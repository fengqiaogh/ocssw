...
int RsViirs::process() {
    ...
    for (int ib = 0; ib < NMBANDS; ib++) {
        ...
        if (ib==12) {
			nc_var.getVar(&ua->in[0][0]);
			resort(ua);
			fill_the_fills(ua);
			nc_var.putVar(&ua->out[0][0]);
        } else {
...
int RsViirs::fill_the_fills(uio* io) {
    const unsigned int filltest = 327680;
