...
!      dump debug output
C      if (abs(xlat-32.64) < 0.05 .AND. abs(xlong-(-114.5827)) < 0.05 .and. gzflg eq 31) then
C       debug = .true.
C       lprint = 1
C      end if
...
c      -- air mass factor
       ...
       xr470 = get_LER470(ilat, ilon, toa_ndvi, scat_ang, xphi)/100.0
...
!      -- over land, skip pixels with SZA > 72.0!
       if (sza > 84.0) go to 10

c--------------------------------------------------------
c   Load Surface Reflectance
c--------------------------------------------------------
c      -- get base surf. reflc. values from surf. coeff. tables and save.
       r412_tbl = get_LER412(ilat, ilon, toa_ndvi, scat_ang, xphi)
       r470_tbl = get_LER470(ilat, ilon, toa_ndvi, scat_ang, xphi)
       r650_tbl = get_LER650(ilat, ilon, toa_ndvi, scat_ang, xphi)
       r865_tbl = get_modis_LER865(ilat, ilon)  ! No 865 yet for VIIRS. Just use MODIS value.

       r412_135 = get_LER412(ilat, ilon, toa_ndvi, 135.0, xphi)
       r470_135 = get_LER470(ilat, ilon, toa_ndvi, 135.0, xphi)
       r650_135 = get_LER650(ilat, ilon, toa_ndvi, 135.0, xphi)
...
c-- test are non-functional. Confirmed w/ Dr. Hsu 2013-03-01 --CB.
       ...
       if (band26.lt.0.0.and.realbuf(11).lt.0.0.and.
     1     realbuf(7).gt.0.0) go to 620
       if (band26.gt.0.0.and.realbuf(11).lt.0.0.and.
     1     realbuf(7).gt.0.0) go to 10
...
c          tau_x650_flag = -999
     ...
     1        r412sv_veg,r470sv_veg,gzflg,outbufvg,tau_x470_flag_veg)
...
c        translate outbufvg back to local variables.
          ...
          if (debug) print *, "veg final: aot550, aot: ", tau550, xtau(1),
     &        xtau(2), xtau(3)
     ...
     &       (outbufvg(2).gt.0.4.and.(realbuf(23)/realbuf(6)).lt.0.7))
     &       tau550 = tau_x470sv96
...
867            continue
     ...
     &     (px_elev > 750.0 .and. (xlat> 14.0. or. xlong< 5.5 .or. xlong> 19.0))
     &     .and. (px_elev > 750.0 .and. (xlat> 14.0 .or. xlong > -6.37))) then
                tau550 = tau_x470sv96_dust
          endif
