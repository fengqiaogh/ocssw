...
!   -- read in base BRDF reflectivitiy @ 650nm from infile.
    ...
    group_name = 'VIIRS_SURFACE_REFLECTANCE'
    status = nf90_inq_ncid(nc_id, group_name, grp_id)
    if (status /= NF90_NOERR) then
        print *, "ERROR: Failed to get ID of group "//trim(group_name)//": ", status
        return
...
!   include 'dffunc.f90'
    ...
    group_name = 'SURFACE_COEFFICIENTS'
    status = nf90_inq_ncid(nc_id, group_name, grp_id)
    if (status /= NF90_NOERR) then
        print *, "ERROR: Failed to get ID of group "//trim(group_name)//": ", status
        return
    ...
    !sds_name = "412_fwd_TP"
    !status = readLER5(start5, edge5, stride5, sds_name, grp_id, coefs412_fwd_tp)
    !if (status < 0) goto 90

    !sds_name = "412_all_TP"
    !status = readLER5(start5, edge5, stride5, sds_name, grp_id, coefs412_all_tp)
    !if (status < 0) goto 90
    ...
    !sds_name = "470_fwd_TP"
    !status = readLER5(start5, edge5, stride5, sds_name, grp_id, coefs470_fwd_tp)
    !if (status < 0) goto 90

    !sds_name = "470_all_TP"
    !status = readLER5(start5, edge5, stride5, sds_name, grp_id, coefs470_all_tp)
    !if (status < 0) goto 90
    ...
    !sds_name = "650_fwd_TP"
    !status = readLER5(start5, edge5, stride5, sds_name, grp_id, coefs650_fwd_tp)
    !if (status < 0) goto 90

    !sds_name = "650_all_TP"
    !status = readLER5(start5, edge5, stride5, sds_name, grp_id, coefs650_all_tp)
    !if (status < 0) goto 90
...
!   -- MODIS, all-angle surface database
    ...
    group_name = 'MODIS_SURFACE_REFLECTANCE'
    status = nf90_inq_ncid(nc_id, group_name, grp_id)
    if (status /= NF90_NOERR) then
        print *, "ERROR: Failed to get ID of group "//trim(group_name)//": ", status
        return
    ...
    !sds_name = "412_bkd"
    !status = readLER2(start2, edges2, stride2, sds_name, grp_id, gref412_bkd)
    !if (status < 0) goto 90
    ...
    !sds_name = "470_bkd"
    !status = readLER2(start2, edges2, stride2, sds_name, grp_id, gref470_bkd)
    !if (status < 0) goto 90
    ...
    !sds_name = "650_bkd"
    !status = readLER2(start2, edges2, stride2, sds_name, grp_id, gref650_bkd)
    !if (status < 0) goto 90
