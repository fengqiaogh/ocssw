...
def main():
    ...
    rp2 = 1e-2
    rp3 = 1e-2
    rikern = mk.inverse(rp0, rp1, rp2, rp3)
    mk.plot("ikern")
    ...
    for x in content_oci: 
        ...
        fname = opt.odir + "/SZ_" + x + mk.name + ".png"      
        print( fname )
        ...
        ro = refl[np.where(mk.wl==670)]
        go = refl[np.where(mk.wl==550)]
        bo = refl[np.where(mk.wl==485)]
        ...
        for i in range(0,1700,10):
            ...
            axs[0,0].plot(mk.wl,irefl[:,i,column], color=colors[int(7*i)%120])    
            axs[1,0].semilogx(mk.sz,psd[:,i,column], color=colors[int(7*i)%120])
