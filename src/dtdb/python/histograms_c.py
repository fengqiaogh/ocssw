...
for x in dtdb_dircontents:
    ...
    if not os.path.isdir(image_path):
        os.mkdir(image_path)
    image_path1 = image_path + "/histograms_c" 
    if (args.alg == "deepblue"):
        outfilename1 = x + "_histc_dba.png" 
    if (args.alg == "darktarget"):
        outfilename1 = x + "_histc_dta.png" 
    if not os.path.isdir(image_path1):
        os.mkdir(image_path1)
    outpath1 = image_path1 + "/" + outfilename1
    image_path2 = image_path + "/maps_c" 
    outfilename2 = x + "_mapc.png" 
    ...
    for wl in range(3, NWL):
        ...
        lrflwl = lrfl490 - np.log10(np.clip(rflwl, a_min=0.0001, a_max=0.999)) - F
#        lrflwl = F
