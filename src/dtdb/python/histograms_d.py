...
for x in dtdb_dircontents:
    ...
    if not os.path.isdir(image_path):
        os.mkdir(image_path)
    if args.side == "left":
        image_path1 = image_path + "/histograms_left" 
    elif args.side == "right":
        image_path1 = image_path + "/histograms_right" 
    else:
        image_path1 = image_path + "/histograms_m" 
    if (args.alg == "deepblue"):
    ...
    image_path2 = image_path + "/maps_m" 
    outfilename2 = x + "_mapm.png" 
    ...
    for vlo in range(32):
        vzalo = vlo*2.5
        condvzalo = vin.vza < vzalo
        vzahi = vzalo+2.5
        condvzahi = vin.vza > vzahi
        print ("Processing VZA range: " + str(vzalo))        
    ...
    #       cgeo = dtdb.interp_geom(vin.rfl,vin.sza,g0,g0,vin.wnd,wl1,fmf,0)/dtdb.interp_geom(vin.rfl,vin.sza,vin.vza,vin.raa,vin.wnd,wl1,fmf,0)
            ...
            lmwlc = np.ma.filled(np.ma.masked_where(condvzalo, lrflwlc))
            lmwlc = np.ma.filled(np.ma.masked_where(condvzahi, lmwlc))
            lmwlc = np.ma.filled(np.ma.masked_where(condszalo, lmwlc))
            lmwlc = np.ma.filled(np.ma.masked_where(condszahi, lmwlc))
            lmwlc = np.ma.filled(np.ma.masked_where(condraalo, lmwlc))
            lmwlc = np.ma.filled(np.ma.masked_where(condraahi, lmwlc))
            lm865 = np.ma.filled(np.ma.masked_where(condvzalo, lrfl865))
            lm865 = np.ma.filled(np.ma.masked_where(condvzahi, lm865))
            lm865 = np.ma.filled(np.ma.masked_where(condszalo, lm865))
            lm865 = np.ma.filled(np.ma.masked_where(condszahi, lm865))
            lm865 = np.ma.filled(np.ma.masked_where(condraalo, lm865))
            lm865 = np.ma.filled(np.ma.masked_where(condraahi, lm865))
            try:
    ...
    #            cp2 = ax2.contourf(lrflwlc, levels=np.arange(-1.0,1.0,0.1), cmap=cmap)
        ...
        title = x + "   SZA:< " + str(szalo) + ", " + str(szahi) + " >   VZA:< " + str(vzalo) + ", " + str(vzahi) + " >   RAA:< " + str(raalo) + ", " + str(raahi) + " >  " + args.side
        fig2.suptitle(title) 
    #        fig2.tight_layout()
    #        fig2.show()
    ...
    #        fig1.tight_layout()
    #        fig1.show()
        plt.savefig(outpath1+"_"+str(vzalo)+".png", facecolor=fig1.get_facecolor()) 
        plt.close(fig1)
        fig1.clf() 
    print (x)
    continue
